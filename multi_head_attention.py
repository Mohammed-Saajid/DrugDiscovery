...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, embedding_dim, num_heads, kernel_size=1,rate=0.5):
    ...
    def build(self, input_shape):
        ...
        self.wq = tf.keras.layers.Dense(self.embedding_dim, activation=tf.keras.activations.silu)
        self.wk = tf.keras.layers.Dense(self.embedding_dim, activation=tf.keras.activations.silu)
        self.wv = tf.keras.layers.Dense(self.embedding_dim, activation=tf.keras.activations.silu)
        ...
        # Output dense layer
        self.dense = tf.keras.layers.Dense(self.embedding_dim, activation=tf.keras.activations.silu)
    ...
    def call(self, q, k, v, mask):
        ...
        output = self.dropout(concat_attention)
...
class PositionwiseFeedforward(tf.keras.layers.Layer):
    ...
    def __init__(self, embedding_dim, dff):
        ...
        self.dense1 = tf.keras.layers.Dense(dff, activation='relu')  # Expansion layer
        self.dense2 = tf.keras.layers.Dense(embedding_dim)  # Projection layer
